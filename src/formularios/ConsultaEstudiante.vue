<template>
  <h2><font-awesome-icon :icon="['fas', 'address-book']" /> Consulta estudiante</h2>
  ...
  <hr>
  <button class="douwnload-btn" @click="downloadPDF" v-if="datos"><font-awesome-icon :icon="['fas', 'download']" />
    descargar informacion</button>
  <div ref="myDiv">
    <div v-if="datos">
      ...
      <div style="display:flex; justify-content:center;">
        <div style="width:100%; margin: 2%; background-color:#fff; box-shadow:0px 2px 6px rgba(0,0,0,0.3); padding:20px;">
          ...
          <div v-for="dato in datos" :key="dato.idEstudiante">
            ...
            <hr>
            <p>Este documento certifica las horas obtenidas por la asistencia a los diferentes espacios organizados por
              Bienestar Universitario. Por lo tanto, se solicita que se proceda a su debida constitución y publicación.
            </p>
        ...
        .then(response => {
          ...
          if (response.data == '') {
            ...
            this.$swal({
              icon: 'error',
              title: 'Oops...',
              text: 'No existe estudiante con el codigo número ' + this.idEstudiante.id,

            });

          }else{
            this.$swal( 'Estudiante encontrado!',
  'puedes ahora continuar con el certificado',
  'success')
          }
        })
        .catch(error => {
